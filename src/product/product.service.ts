...
export class ProductService {
    ...
    async removeProduct(prodId){
        try {
            ...
            const productForRemove= await this.productModel.findById({_id:ObjectId})
    ...
    async updateProduct(updateProductDto,image,id){
        try {
            ...
            const {productName,modelName,specification,color,price}=updateProductDto
            console.log("p name",productName);
            console.log(image);
            
            const productForUpdate= await this.productModel.findById({_id:new mongoose.Types.ObjectId(id)})
            if(!productForUpdate){
                return {status:HttpStatus.BAD_GATEWAY,
                    message:"Product Not found"}
            }
            const updateProduct= await this.productModel.findByIdAndUpdate({_id:new mongoose.Types.ObjectId(id)},{
                productName,
                modelName,
                specification,
                color,
